var personagens = document.getElementById("personagem")
var input = document.querySelector("input")
var verMais = 20

// hash

const privateKey = "c7bd21bffee4fc1e8b27714f60748f8dfd399d3e"
const publicKey = "107e9993c2603fb746fa4f7beadd43ea"

function creatHash(timeStamp){
...
const herosFromFedd = heros => heros.data.results.map(item =>`
    ...
    <img id="${item.name}" class="card-image" alt=${item.name} 
    src= "${item.thumbnail.path}${'.'}${item.thumbnail.extension}"/>
    <h2 id="${item.name}" class="card-title">${item.name}</h2>
    </li>
    ` ).join('')
...
const earlyFeed = async () => {
    const heros = await getPosts(`orderBy=name&limit=${verMais}`);
    const postsTemplate = herosFromFedd(heros)
    personagens.innerHTML = postsTemplate;
    busca()
}
...
earlyFeed();
input.addEventListener('input', modifyInputValue)
function vermais(){
    verMais +=20
    earlyFeed()
}
...
card.map((evt)=>{
    ...
    evt.addEventListener("click", (evento)=>{
       
        searchModal(evento.target.id)
        
        console.log(evento.target.id)
    })
    
})
const searchModal = async (search) => {
...
function MudarDisplay(){
    ...
    section.style.display = arrayListAtributs[currentListAtributs]
    currentListAtributs++
}
